...
import { generateWAMessageFromContent, generateMessageIDV2, jidEncode } from '@whiskeysockets/baileys';

export const lastDiag = [];

const toFullJid = (d) => {
  if (!d) return null;
  if (d.jid) return d.jid; // axleys sudah kasih .jid
  if (d.user) return jidEncode(d.user, d.server || 'lid', d.device ?? 0); // baileys ori: rakit sendiri
  return null;
};
  ...
  for (const jid of candidates) {
    try {
      ...
      if (devices?.length) {
        ...
        const full = toFullJid(lid || devices[0]);
        if (full) return full;
      }
